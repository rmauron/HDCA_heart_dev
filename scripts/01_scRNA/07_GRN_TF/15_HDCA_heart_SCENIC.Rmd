...
exprMat_filtered <- exprMat[VariableFeatures(CM),] #keeps only the highly variable genes
dim(exprMat_filtered)
...
```{r}
data_cell <- SetIdent(data_cell, value = "clusters_subset_split") #clusters_subset_split (for CM), clusters_subset (for EN, FB, IN)
...
all_clusters <- levels(data_cell$clusters_subset_split) #clusters_subset_split (for CM), clusters_subset (for EN, FB, IN)
...
#skip this filtering for testing purposes
...
#Save plot
pdf(paste0(myfolder, "scenic/figures/", data_cell, "_", cluster, ".pdf"), width = 7, height = 7)
p
dev.off()
