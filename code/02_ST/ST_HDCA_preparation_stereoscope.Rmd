...
```{r}
# ST_decon_file <- readr::read_tsv("~/hdca_DevHeart/data/ST/hdca_heart_ST_data.tsv")
# #colnames(ST_decon_file) --> genes
# #ST_decon_file[,1] --> barcodes
# #rownames(ST_decon_file) --> 1,2,3,4,5,...
# ST_decon_file[1:10, 1:10]
# 
# dim(ST_decon_file)
# sort(colnames(ST_decon_file))
```



```{r}
# stereoscope_data_ST <- as.matrix(ST@assays$Spatial@counts)
# 
# colnames(ST@assays$Spatial@counts)
# sort(rownames(ST@assays$Spatial@counts))
# 
# 
# common <- intersect(sort(colnames(ST_decon_file)), sort(rownames(ST@assays$Spatial@counts)))
# length(common)
