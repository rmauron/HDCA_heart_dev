...
## UMAP all clusters embedded (Supp. Fig. 15E)
...
### VCS-Ion channel (Extended Fig. 3F)
...
#### HBA1 across all CM clusters (without exclusions, unordered) (Supp. Fig. 16C)
...
#### HBA1 across all EN clusters (without exclusions, unordered) (Supp. Fig. 16C)
...
### HBA1 across all FB clusters (again without exclusions, unordered) (Supp. Fig. 16C)
...
#### Data - all clusters (clusters_louvain_oi) (Supp. Fig. 15F)
...
#### HBA1 across all HL clusters (without exclusions, unordered) (Supp. Fig. 15F)
...
```{r}
number_cells <- as.matrix(table(data$clusters_subset_split, data$orig.ident))
number_cells_annot <- number_cells
rownames(number_cells_annot) <- cells_clusters[as.character(rownames(number_cells_annot))]

# Save the detable as a csv file.
write.csv2(number_cells_annot, paste0(myfolder, "supplementary_tables/extended_table_11_number_cells_DL_per_orig_ident.csv"))
```

```{r}
number_cells <- as.matrix(table(data$clusters_subset_split, data$sampleID))
number_cells_annot <- number_cells
rownames(number_cells_annot) <- cells_clusters[as.character(rownames(number_cells_annot))]

# Save the detable as a csv file.
write.csv2(number_cells_annot, paste0(myfolder, "supplementary_tables/extended_table_11_number_cells_DL_per_sampleID.csv"))
```

HL

```{r}
number_cells <- as.matrix(table(data$high_level_clusters_removed, data$orig.ident))
number_cells_annot <- number_cells
#rownames(number_cells_annot) <- cells_clusters[as.character(rownames(number_cells_annot))]

# Save the detable as a csv file.
write.csv2(number_cells_annot, paste0(myfolder, "supplementary_tables/extended_table_11_number_cells_HL_per_orig_ident.csv"))
```

```{r}
number_cells <- as.matrix(table(data$high_level_clusters_removed, data$sampleID))
number_cells_annot <- number_cells
#rownames(number_cells_annot) <- cells_clusters[as.character(rownames(number_cells_annot))]

# Save the detable as a csv file.
write.csv2(number_cells_annot, paste0(myfolder, "supplementary_tables/extended_table_11_number_cells_HL_per_sampleID.csv"))
```


#### WNT, BMP, TGFb, axes expression "EN_17", "EN_7", "FB_4", "FB_13", "FB_11" (Fig. 7E)
...
#### Valve pathological gene expression "EN_17", "EN_7", "FB_4", "FB_13", "FB_11" (Fig. 7F)
...
#### "EN_7", "EN_17", "FB_4", "FB_11", "FB_13", "CM_8-2" (Fig. 5C)

```{r}
clusters_to_analyze <- c("EN_17", "EN_7", "FB_4", "FB_13", "FB_11", "CM_8-2")
...
```{r}
my_markers <- c("PTHLH", "WNT4", "DKK2", "CNR1", "LGR5", "BMP4")
```
...
gg_mat$name <- factor(gg_mat$name, levels = c("EN_7", "EN_17", "FB_4", "FB_11", "FB_13", "CM_8-2"))
...
pdf(paste0(myfolder, "EN_7_EN_17_FB_4_FB_11_FB_13_CM_8-2_markers_niche_gg_with_norm.pdf"), width = 3, height = 2.5)
p
dev.off()
```
...
#### FeaturePlot JAG1 NOTCH3

```{r}
pdf(paste0(myfolder, "FeaturePlot_JAG1.pdf"), width = 20, height = 15)
FeaturePlot(data_curated, reduction = "umap_oi", features = 'JAG1', order = TRUE)
dev.off()

pdf(paste0(myfolder, "FeaturePlot_NOTCH3.pdf"), width = 20, height = 15)
FeaturePlot(data_curated, reduction = "umap_oi", features = 'NOTCH3', order = TRUE)
dev.off()

pdf(paste0(myfolder, "FeaturePlot_CD34.pdf"), width = 20, height = 15)
FeaturePlot(data_curated, reduction = "umap_oi", features = 'CD34', order = TRUE)
dev.off()
```


#### Dotplot JAG1 NOTCH3 (Supp. Fig. 3D)

```{r}
# specific list of marker to project the dotplot on
my_markers <- c("JAG1", "NOTCH3")
...
pdf(paste0(myfolder, "HL_dotplot_JAG1_NOTCH3.pdf"), width = 13, height = 4)
print(p)
dev.off()
```
