...
### Add cl4, cl8 and cl19 subclusters (Supp. Fig. 16A)
...
### DEG CM - top5 ordered (Extended Fig. 3A)
...
### DEG CM selection of markers (not split & all) (Supp. Fig. 16D)
...
#### Avg expression (Fig. 3E)

```{r}
clusters_to_analyze <- c("18", "4-2", "4-1", "6")

data_sub <- CM[, CM$clusters_subset_split %in% clusters_to_analyze]

table(data_sub$clusters_subset_split)
table(data_sub$cell.types)
```
...
mat <- AverageExpression(object = data_sub, features = my_markers, group.by = 'cell.types')$RNA
...
gg_mat$name <- factor(gg_mat$name, levels = c("AVB-BB_CM", "PF_CM", "TsPF_CM", "vCM_4"))
...
pdf(paste0(myfolder, "cardiomyocytes/avg_expr_markers_CM_subset_selection_18_4-2_4-1_6.pdf"), width = 3, height = 2.7)
p
dev.off()
```
...
#### Avg expression  (Fig. 3D)

```{r}
clusters_to_analyze <- c("19-2", "19-1", "12", "9", "14")
...
gg_mat$name <- factor(gg_mat$name, levels = c("SAN_CM", "AVN_CM", "Cond_aCM", "Left_aCM", "Right_aCM"))
...
pdf(paste0(myfolder, "cardiomyocytes/avg_expr_markers_CM_subset_selection_19-2_19-1_12_9_14.pdf"), width = 3, height = 2.5)
p
dev.off()
```


### DEG CM ADRB1, CHRM2 (Fig. 4G)
