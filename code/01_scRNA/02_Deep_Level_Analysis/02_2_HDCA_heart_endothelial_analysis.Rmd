...
### DEG EN - top5 ordered (Extended Fig. 5A)
...
### DEG EN-7, EN-17, EN-18 - top10 ordered (Extended Fig. 5F)
...
### DEG EN-1, EN-3, EN-6, EN-12 - top10 Supp Fig. 11C)
...
# reorder the cell types desired
DGE_DATA$cell.types <- factor(DGE_DATA$clusters_subset, 
...
### DEG EN-2, EN-10 - top10 Supp Fig. 11B)

```{r}
values_to_keep <- c("10", "2")
...
pdf(paste0(myfolder, "endothelial/DEG_top5_EN2_10.pdf"), width = 5, height = 10)
par(mfrow=c(1,1), mar=c(3,6,2,5))
DotPlot(DGE_DATA, features = top10 %>% pull(gene) %>% unique()) +
  coord_flip() +
  theme(axis.text.y = element_text(angle = 0, hjust = 1, vjust = 0.5)) +
...
### DEG EN selection of markers (Supp. Fig. 16D)
