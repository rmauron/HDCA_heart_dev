...
```{r}
annotation <- read.csv2("/home/rstudio/project/data/metadata/HDCA_heart_SC annotations_EN_240115.csv", header = 1, sep = ";")
...
### DEG EN-1, EN-3, EN-6, EN-12 - top10 ordered
...
# reorder the cell types desired
DGE_DATA$cell.types <- factor(DGE_DATA$cell.types, 
                                              levels = c("Endoc_EC_1", "Endoc_EC_3", "Endoc_EC_4", "Endoc_EC_2"))
...
pdf(paste0(myfolder, "endothelial/DEG_top10_EN1_3_6_12.pdf"), width = 5, height = 10)
par(mfrow=c(1,1), mar=c(3,6,2,5))
DotPlot(DGE_DATA, features = top10 %>% pull(gene) %>% unique()) +
  coord_flip() +
  theme(axis.text.y = element_text(angle = 0, hjust = 1, vjust = 0.5)) +
...
pdf(paste0(myfolder, "endothelial/DEG_top10_EN2_10.pdf"), width = 5, height = 5)
par(mfrow=c(1,1), mar=c(3,6,2,5))
DotPlot(DGE_DATA, features = top10 %>% pull(gene) %>% unique()) +
  coord_flip() +
  theme(axis.text.y = element_text(angle = 0, hjust = 1, vjust = 0.5)) +
...
### DEG EN selection of markers EN_18, EN_17 and EN_7 comparison (split & selection)
...
data_EN$cell.types <- factor(data_EN$cell.types, 
                                              levels = c("AtrSept_EC", "IF_VEC", "OF_VEC"))

data_EN <- SetIdent(data_EN, value = "cell.types")
...
pdf(paste0(myfolder, "endothelial/dotplot_markers_EN_subset_selection_18_17_7.pdf"), width = 4, height = 10)
print(p)
dev.off()
...
```{r}
cell_HL_DL <- table(EN$cell.types, EN$clusters_subset)

# Save markers_table
write.csv(cell_HL_DL, file = paste0(myfolder, "endothelial/", "cell_HL_DL", "_EN", ".tsv"))
```
