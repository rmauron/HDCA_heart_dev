...
If interested in only 1 cluster, can take all the cells:
Ran independantly for CM19-2, CM19-1, CM4-2, EN7, EN17, EN18, FB14, FB4
...
If interested in several clusters, can take even number of cells:

```{r}
# ### OR this to have the same number of cells in both clusters
# # Assuming your Seurat object is named data_cell
# data_cell <- CM
# cluster <- c("19-1", "19-2")
# data_sub <- data_cell[, data_cell$clusters_subset_split %in% cluster]
# 
# # Find the cells belonging to the specified clusters
# cells_to_downsample_19_2 <- WhichCells(data_sub, ident = "19-2")
# cells_to_downsample_19_1 <- WhichCells(data_sub, ident = "19-1")
# 
# # Determine the minimum number of cells in the two clusters
# min_cells <- min(length(cells_to_downsample_19_1), length(cells_to_downsample_19_2))
# 
# # Downsample the specified number of cells for each cluster
# set.seed(1)  # for reproducibility
# data_sub_19_2 <- subset(data_sub, cells = sample(cells_to_downsample_19_2, min_cells))
# data_sub_19_1 <- subset(data_sub, cells = sample(cells_to_downsample_19_1, min_cells))
# 
# # Combine the downsampled subsets
# downsampled_data_sub <- merge(data_sub_19_1, data_sub_19_2)
# 
# data_sub <- downsampled_data_sub
# 
# # Check the number of cells in each cluster
# table(data_sub$clusters_subset_split)
# 
# # Remove temp objects
# rm(downsampled_data_sub, data_sub_19_1, data_sub_19_2, cells_to_downsample_19_1, cells_to_downsample_19_2, min_cells)
```
...
exprMat_filtered <- exprMat[VariableFeatures(FB),] #keeps only the highly variable genes
dim(exprMat_filtered)
...
```{r}
data_cell <- SetIdent(data_cell, value = "clusters_subset") #clusters_subset_split (for CM), clusters_subset (for EN, FB, IN)
data_cell$clusters_subset <- factor(data_cell$clusters_subset, 
                                              levels = c("7", "3", "18", "14", "5", "9", "19", "2", "12", "4", "13", "11", "17", "8", "1", "6", "15", "20"))

#CM
# c("1", "2", "3", "4-1", "4-2", "5", "6", "7", "8-1", "8-2", "8-3", "9", "10", "12", "14", "17", "19-1", "19-2")
#EN
# c("1", "12", "3", "6", "18", "17", "7", "19", "15", "13", "10", "2", "5", "21", "16", "20", "11", "14")
#FB
# ("7", "3", "18", "14", "5", "9", "19", "2", "12", "4", "13", "11", "17", "8", "1", "6", "15", "20")

# Identify all other clusters
all_clusters <- levels(data_cell$clusters_subset) #clusters_subset_split (for CM), clusters_subset (for EN, FB, IN)
...
```{r}
# #Read regulons from the temporary file from scenic output
# table_reg <- read.csv("./output/Step2_regulonTargetsInfo.tsv", sep = "\t")
# motifEnr <- read.csv("./output/Step2_MotifEnrichment.tsv", sep = "\t")
# 
# 
# #Save regulons from temporary file in own directory
# write.csv(table_reg, file = paste0(myfolder, "scenic/regulons/regulonTargetsInfo_", data_cell, "_", cluster, ".tsv"))
# #Read regulons saved in own directory to repeat analysis if needed
# #table_reg <- read.csv(paste0(myfolder, "scenic/regulons/regulonTargetsInfo_CM_19-2.tsv"), sep = "\t")
# 
# table_reg %>% filter(highConfAnnot) -> table_reg_conf
# table_reg_conf
# #table_reg_conf %>% filter(TF %in% c("KLF4", "NFE2L2", "JUND")) -> table_reg_conf_sm #subset from the available TF
```
...
set.seed(1) #TO FIX seed in layout

layout <- layout.fruchterman.reingold(tg, niter=400)
unique_TF <- unique(regulons$TF)


# With fixed size for node_point, all the nodes are the same size
# ggraph(tg, layout = layout) +
#   geom_edge_link0(color = "grey20", alpha = 0.25) +
#   geom_node_point(aes(size = 500, 
#                       fill = ifelse(name %in% unique_TF, 'orange', 'turquoise'), color = 'black'), shape = 21, stroke = 0.3) +
#   geom_node_text(aes(label = name, size = size * 20), 
#                  color = "black", repel = F) +
...
# 
# 
# # With vectorized size for node_point, TF nodes are proportional to their connectivities
# ggraph(tg, layout = layout) +
#   geom_edge_link0(color = "grey20", alpha = 0.25) +
#   geom_node_point(aes(size = size*50, 
