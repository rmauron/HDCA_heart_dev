...
# HDCA heart complementary figures
...
# Save the detable as a csv file.
write.csv2(detable, paste0(myfolder, "complementary/detable_general.csv"))
...
pdf(paste0(myfolder, "complementary/DGE_general.pdf"), width = 15, height = 50)
par(mfrow=c(1,1), mar=c(3,6,2,5))
DotPlot(DGE_DATA,  features = top5 %>% pull(gene) %>% unique()) +
  coord_flip() +
  theme(axis.text.y = element_text(angle = 0, hjust = 1, vjust = 0.5)) +
...
DGE_cells <- lapply(names(sample_size), function(x){
  set.seed(1)
  sample(colnames(data)[data$groups == x], size = sample_size[x])
  })
DGE_cells <- unlist(DGE_cells)
...
pdf(paste0(myfolder, "complementary/DGE_general_age_with_lines.pdf"), width = 25, height = 75)
par(mfrow=c(1,1), mar=c(3,6,2,5))
DotPlot(DGE_DATA,  features = top5 %>% pull(gene) %>% unique()) +
  coord_flip() +
  theme(axis.text.y = element_text(angle = 0, hjust = 1, vjust = 0.5)) +
  ...
  #labs(x = "", y = "") +
  theme(axis.text.x = element_text(angle = 90, hjust = 0.5)) +
  #abline(v=cumsum(c(table(sub("^(cluster_\\d+).*", "\\1",names(table(detable$cluster))))))+0.5)
dev.off()
```
...
DGE_cells <- lapply(names(sample_size), function(x){
  set.seed(1)
  sample(colnames(data)[data$group_refined == x], size = sample_size[x])
  })
DGE_cells <- unlist(DGE_cells)
...
DGE_cells <- lapply(names(sample_size), function(x){
  set.seed(1)
  sample(colnames(CM)[CM$group_refined == x], size = sample_size[x])
  })
DGE_cells <- unlist(DGE_cells)
...
DGE_cells <- lapply(names(sample_size), function(x){
  set.seed(1)
  sample(colnames(EN)[EN$group_refined == x], size = sample_size[x])
  })
DGE_cells <- unlist(DGE_cells)
...
DGE_cells <- lapply(names(sample_size), function(x){
  set.seed(1)
  sample(colnames(FB)[FB$group_refined == x], size = sample_size[x])
  })
DGE_cells <- unlist(DGE_cells)
...
p <- DotPlot(CM_subset, features = my_markers) +
  theme(axis.text.y = element_text(angle = 0, hjust = 1, vjust = 0.5)) +
  scale_color_gradientn(colours = RColorBrewer::brewer.pal(n = 11, name = "RdBu") %>% rev()) +
  scale_x_discrete() +
...
pdf(paste0(myfolder, "complementary/DGE_CM_ion_channel.pdf"), width = 13, height = 8)
print(p)
dev.off()
...
pdf(paste0(myfolder, "complementary/DGE_CM_MY.pdf"), width = 5, height = 8)
print(p)
dev.off()
...
```{r}
pdf(paste0(myfolder, "cardiomyocytes/nFeatures_MYH6.pdf"), width = 10, height = 10)
FeaturePlot(CM_subset, reduction = "umap_subset", features = "MYH6", raster = FALSE)
dev.off()
```
...
```{r}
pdf(paste0(myfolder, "cardiomyocytes/nFeatures_MYH7.pdf"), width = 10, height = 10)
FeaturePlot(CM_subset, reduction = "umap_subset", features = "MYH7", raster = FALSE)
dev.off()
```
...
DGE_cells <- lapply(names(sample_size), function(x){
  set.seed(1)
  sample(colnames(CM)[CM$groups == x], size = sample_size[x])
  })
DGE_cells <- unlist(DGE_cells)
...
pdf(paste0(myfolder, "complementary/DGE_CM_age.pdf"), width = 25, height = 8)
par(mfrow=c(1,1), mar=c(3,6,2,5))
...
pdf(file = paste0(myfolder, "complementary/EN_feature_vln.pdf"),  width = 15, height = 4)
...
```{r}
# Create a single violin plot
pdf(file = paste0(myfolder, "complementary/CM_feature_vln.pdf"),  width = 15, height = 4)
...
```{r}
# Create a single violin plot
pdf(file = paste0(myfolder, "complementary/FB_feature_vln.pdf"),  width = 15, height = 4)
...
```{r}
# Create a single violin plot
pdf(file = paste0(myfolder, "complementary/data_feature_vln.pdf"),  width = 15, height = 4)
